...
@Component({
  selector: 'app-modify-basket',
  standalone: false,
  templateUrl: './modify-basket.component.html',
  styleUrl: './modify-basket.component.css',
})
export class ModifyBasketComponent {
  form = this.fb.group({
      ...
      next: (created) => {
        ...
        if (selected.length > 0) {
          ...
          selected.forEach((articleId) => {
            this.basketService
              .assignArticleToBasket(articleId, 1, 'ACHETER', basketId)
              .subscribe();
          });
        }
